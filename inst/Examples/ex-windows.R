...
b1 = gbutton("galert", cont=g1, handler = function(h,...) {
 galert("galert dialog","for quick transient messages")
})
...
b3 = gbutton("gconfirm", cont=g1, handler = function(h,...) {
  gconfirm("Unfortunately the handler doesn't work without an intrusive hack.", parent = b2, handler = function(h,...) {
    gmessage("you clicked ok", parent = b3)
  })
})

b4 = gbutton("ginput", cont=g1, handler = function(h,...) {
  ginput("Unfortunately the handler doesn't work without an intrusive hack.", parent = b3, handler = function(h,...) {
